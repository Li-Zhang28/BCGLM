...
###similarity matrix
  ...
  for (i in 1:(ncol(K.taxa)-1))
    ...
    {
      if(abs(K.taxa[i,j]) > quantile(abs(taxa))[3]) ###0.18
      {
        node1 = c(node1,i);
        node2 = c(node2,j);
        w = c(w, K.taxa[i,j])
...
f4= brm(fm, data=dat1, family=gaussian(), prior=bp4, stanvars=stanvars,control = list(adapt_delta = 0.99,max_treedepth= 18),
        chains=4, iter=4000)
samples4 = fixef(f4)
samples4=samples4[-1,]
